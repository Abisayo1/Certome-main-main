...
import { BiLogoInstagram, BiLogoTiktok, BiPhoneCall, BiSolidEnvelope } from "react-icons/bi";
      ...
      <div className="right-col my-20 xl:my-0 p-5 sm:p-20 w-full xl:w-[60%] " >
        <div className="inputs grid sm:grid-cols-2 gap-10 ">
          <div className="flex flex-col" >
            ...
            <input type="text" className=" border-b-2 py-2 outline-none " placeholder="John" id="FirstName" />
          </div>
          <div className="flex flex-col" >
            <label className="font-bold" htmlFor="LastName">Last Name</label>
            <input type="text" className=" border-b-2 py-2 outline-none " placeholder="Doe" id="LastName" />
          </div>
          <div className="flex flex-col" >
            <label className="font-bold" htmlFor="Email">Email</label>
            <input type="email" className=" border-b-2 py-2 outline-none " placeholder="123@gmail.com" id="Email" />
          </div>
          <div className="flex flex-col" >
            <label className="font-bold" htmlFor="PhoneNumber">Phone Number</label>
            <input type="text" className=" border-b-2 py-2 outline-none " placeholder="+1 234 567 890" id="PhoneNumber" />
          </div>
        </div>
        
        <div className="text-lg mt-10 mb-5 font-bold">Select Subject?</div>
        <div className="radio flex flex-wrap justify-between">
          {['General Inquiry', 'Support Request', 'Business Inquiry', 'Other'].map((subject, index) => (
            <div key={index} className="flex items-center">
              <input className="mr-2 cursor-pointer accent-primary" type="radio" name="subject" id={`subject-${index}`} />
              <label className="cursor-pointer" htmlFor={`subject-${index}`}>{subject}</label>
            </div>
          ))}
        </div>
        <div className="message flex flex-col my-10 ">
          <label htmlFor="message">Message</label>
          <input type="text" className=" border-b-2 py-2 outline-none " placeholder="Write your message" id="message" />
        </div>
        <div className="flex justify-end">
          <button className="bg-[#060660] text-white px-6 py-3 rounded-lg font-bold">Send Message</button>
        </div>
      </div>
    </div>
  );
